...
import torchmetrics
from .BayesianCFormKlLayers import LinearGroupNJ
import torch
import torch.nn as nn
import torch.nn.functional as F
...
# simple 2-layer dense network
class BayesianDenseThreeLayerCateg(nn.Module):
    ...
    def __init__(self,in_features, out_features, embedding_sizes, n_cont, d_rate=0.1):
        ...
        self.d_rate = d_rate
        # define a fully connected layer
        self.l1 = LinearGroupNJ(self.n_emb + self.n_cont ,512)
        # define the dropout layer
        self.drop = nn.Dropout(d_rate)
        # define a second FCN
        self.l2 = LinearGroupNJ(512,256)
        # define the second dropout layer
        self.l3 = LinearGroupNJ(256,128)
        self.drop = nn.Dropout(d_rate)
        # dropout for embedding
        self.emb_drop = nn.Dropout(0.5)
        # define the final prediction layer
        self.ol = LinearGroupNJ(128,self.o_dim)
        # batch norm layers, guideline for batchnorm: https://stackoverflow.com/questions/39691902/ordering-of-batch-normalization-and-dropout
        self.bn1 = nn.BatchNorm1d(512)
        self.bn2 = nn.BatchNorm1d(256)
        self.bn3 = nn.BatchNorm1d(128)
        self.kl_list = [self.l1, self.l2, self.l3, self.ol]
...
class MaskedDenseThreeLayerCateg(nn.Module):
    ...
    def __init__(self,in_features, out_features, embedding_sizes, n_cont, d_rate=0.1):
        ...
        # Learnable mask for continuous and categorical features
        self.mask_emb = nn.Parameter(torch.ones(self.n_emb) * 0.9)  # initialized close to 1
        self.mask_cont = nn.Parameter(torch.ones(self.n_cont) * 0.9)  # initialized close to 1
